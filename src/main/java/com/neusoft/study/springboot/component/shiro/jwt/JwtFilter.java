...
//             mustLoginFlag = true 开启任何请求必须登录才可访问
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        RedisServiceUtil redisServiceUtil = SpringUtils.getBean(RedisServiceUtil.class);
        ...
        if (redisServiceUtil.hasKey(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account)) {
            ...
            if (jwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                Long refreshTokenExpireTime = userProperties.getRefreshTokenExpireTime();
                // 设置RefreshToken中的时间戳为当前最新时间戳，且刷新过期时间重新为30分钟过期(配置文件可配置refreshTokenExpireTime属性)
                redisServiceUtil.set(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account, currentTimeMillis, refreshTokenExpireTime);
                ...
                HttpServletResponse httpServletResponse = WebUtils.toHttp(response);
                httpServletResponse.setHeader(userProperties.getTokenKey(), token);
                httpServletResponse.setHeader("Access-Control-Expose-Headers", userProperties.getTokenKey());
                return true;
            }
        }
        return false;
